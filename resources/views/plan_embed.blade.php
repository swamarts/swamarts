...
<head>
    ...
    <!-- Font -->
    <link
        href="https://fonts.googleapis.com/css?family=Poppins:400,400i,600,600i,700,700i,800,800i&amp;subset=cyrillic,cyrillic-ext,greek,greek-ext,latin-ext,vietnamese"
        rel="stylesheet">

    <!-- jQuery -->
    <script type="text/javascript" src="{{ env('APP_URL') }}/core/js/jquery-3.6.0.min.js"></script>

    <!-- Bootstrap -->
    <link rel="stylesheet" type="text/css" href="{{ env('APP_URL') }}/core/bootstrap/css/bootstrap.min.css">
    <script type="text/javascript" src="{{ env('APP_URL') }}/core/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Numeric -->
    <script type="text/javascript" src="{{ env('APP_URL') }}/core/numeric/jquery.numeric.min.js"></script>


    <!-- AcelleMail -->
    <link rel="stylesheet" type="text/css" href="{{ env('APP_URL') }}/core/css/app.css">
            ...
            <div class="col-md-10">
                ...
                <div class="free-plan-tag">
                    ...
                    <ul class="nav nav-tabs nav-fill justify-content-center" id="myTab" role="tablist">
                        ...
                        <li class="nav-item currencyDrop" role="presentation">
                            <select class="nav-link me-3" name="currency" id="currency_convert">
                                @foreach ($currency as $item)
                                    <option value="{{ $item->code }}" data-icon="{{ $item->format }}"
                                        data-localisation_factor="{{ $item->localisation_factor }}" data-symbal="{{$item->usd_value}}"
                                        {{-- data-symbal="{{ str_replace('{PRICE}', '', $item->format) }}" --}}
                                        >
                                            ...
                                            <div data-url="{{ action('SubscriptionController@orderBox', ['plan_uid' => $plan->uid]) }}"
                                                ...
                                                <div style="height: 250px">
                                                    ...
                                                    @if (!$plan->isFree() && is_countable(json_decode($plan->tier)) && count(json_decode($plan->tier)) != 0)
                                                        ...
                                                        <input type="range" class="w-100" style="accent-color: #138b8b;" min="0" max="{{ count(json_decode($plan->tier)) - 1 }}" value="${credits}" name="discount_credits" id="discount_credits{{ $plan->uid }}" />
                                                        ...
                                                        `;
                                                            ...
                                                            $('#discount_credits{{ $plan->uid }}').on("change mousemove", function() {
                                                                localStorage.setItem("plan_tier", $('#discount_credits{{ $plan->uid }}').val());  
                                                ...
                                                <div>
                                                    <div style="vertical-align:bottom">
                                                        <button
                                                            class="btn fw-600 w-100 btn-primary rounded-3 d-block py-2 shadow-sm "
                                                            onclick="selectPlan('{{ $plan->uid }}', 'month')">
                                                        ...
                                                        {{-- <a link-method="POST"  target="_blank" onclick="this.href='/account/subscription/init?plan_uid={{$plan->uid}}&tier='+document.getElementById('discount_credits_month').value"
                                    ...
                                    @foreach ($plans as $key => $plan)
                                        @if ($plan->displayFrequencyTime() == 'month')
                                            <div data-url="{{ action('SubscriptionController@orderBox', ['plan_uid' => $plan->uid]) }}"
                                                ...
                                                <div style="height: 250px">
                                                    ...
                                                    @if (is_countable(json_decode($plan->tier)) && count(json_decode($plan->tier)) != 0)
                                                        <script>
                                                            ...
                                                            html = `
                                                        <h4>Subscribers : @if ($plan->displayTotalSubscribers() != 'Unlimited')<span id="newSubscriber{{ $plan->uid }}">${planTier['{{ $plan->uid }}'][credits]['subscriber_max']/1000}K</span>@else {{ $plan->displayTotalSubscribers() }} @endif</h4>
                                                        <input type="range" class="w-100" style="accent-color: #138b8b;" min="0" max="{{ count(json_decode($plan->tier)) - 1 }}" value="${credits}" name="discount_credits_month" id="discount_credits_month{{ $plan->uid }}" />
                                                        ...
                                                        `;
                                                            ...
                                                            $('#discount_credits_month{{ $plan->uid }}').on("change mousemove", function() {
                                                                localStorage.setItem("plan_tier", $('#discount_credits_month{{ $plan->uid }}').val());  
                                                                if ("{{ $plan->displayTotalQuota() }}" != 'Unlimited') {
                                                                    $('#newEmails{{ $plan->uid }}').html(convertAmount(planTier['{{ $plan->uid }}'][$(this)
                                                                        .val()
                                                                    ]['email_max'] / 1000))
                                                ...
                                                <div>
                                                    <div style="vertical-align:bottom">
                                                        {{-- {{ action('SubscriptionController@init', [ 'plan_uid' => $plan->uid,'tier' => "" ]) }} --}}
                                                        {{-- <a link-method="POST" target="_blank" onclick="this.href='/account/subscription/init?plan_uid={{$plan->uid}}&tier='+document.getElementById('discount_credits_month').value"
                                                    class="btn fw-600 btn-primary rounded-3 d-block py-2 shadow-sm">
                                                    @if ($plan->isFree() || $plan->hasTrial())
                                                        {{ trans('messages.plan.select') }}
...
<script>
    ...
    function selectPlan(uid, type) {
        localStorage.setItem("embad_code", true);
        localStorage.setItem("plan_id", uid);
        localStorage.setItem("plan_type", type);
        localStorage.setItem("currency", country_code);
        if ($('#discount_credits' + (type == 'month' ? '_month' + uid : uid)).length) {
            localStorage.setItem("plan_tier", $('#discount_credits' + (type == 'month' ? '_month' + uid : uid)).val());
        }
        window.open("{{ route('customer.register') }}", '_blank');
    }
