...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-8 col-lg-8">
                ...
                @if ($subscription->isActive())
                    ...
                    @if (\Auth::user()->customer->can('changePlan', $subscription))
                        <a
                            ...
                            class="btn btn-danger change_plan_button me-1"
                            data-size="sm"
                        >
            ...
            <h2 class="text-semibold">{{ trans('messages.ip_address') }}</h2>

            <div class="sub-section row">
                @if ($plan->hasIpaddress())
                    <div class="col-md-6">Dedicated IP: </div>
                    <div class="col-md-6">
                        {!!$plan->ipAddressValue()!!}
                    </div>
                @else
                    <div class="col-md-12">
                        @if (!empty($ipaddress) && $ipaddress->status != 'cancel')
                            Dedicated IP: {{$ipaddress->ip_address}} 
                            <br>
                                <a link-method="POST" link-confirm="{{ trans('messages.ipaddress.cancel_now.confirm') }}"
                                    href="{{ action('SubscriptionController@cancelIpNow', $ipaddress->id) }}"
                                    class="btn btn-danger me-2">
                                    {{ trans('messages.cancel') }} {{ trans('messages.ip') }}
                                </a>
                        @else
                            <a link-method="POST" link-confirm="{{ trans('messages.ip_message_purchase') }}"
                                href="{{ action('SubscriptionController@checkoutIp') }}"
                                class="btn btn-danger me-2">
                                {{ trans('messages.ip_purchase') }}
                            </a>
                        @endif
                    </div>
                @endif
            ...
            <div class="sub-section">
                <div class="">
                    <div class="">
                        ...
                        @include('plans._details', ['plan' => $plan, 'subscritpion' => $subscription, 'currency' => $currency])
                    </div>
                </div>
            </div>
        </div>
