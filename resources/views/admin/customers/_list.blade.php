@if ($customers->count() > 0)
    ...
    >
        @foreach ($customers as $key => $customer)
            <tr>
                <td width="1%">
                    <img width="50" class="rounded-circle me-2" src="{{ !empty($customer->user) ? $customer->user->getProfileImageUrl() : '' }}" alt="">
                </td>
                <td>
                    <h5 class="m-0 text-bold">
                        <a class="kq_search d-block" href="{{ action('Admin\CustomerController@edit', $customer->uid) }}">{{ !empty($customer->user) ? $customer->displayName() : '' }}</a>
                    </h5>
                    <span class="text-muted kq_search">{{ !empty($customer->user) ? $customer->user->email : '' }}</span><br>
                    ...
                    @if (Auth::user()->can('delete', $customer) ||
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                ...
                                @if (!empty($customer->getCurrentActiveSubscription()->plan) && $customer->getCurrentActiveSubscription()->plan->has_ip_address)
                                    @if ($customer->isIpaddress())
                                        <li>
                                            <a class="dropdown-item disabled">
                                                <span class="material-symbols-rounded">public</span>{{ trans('messages.ip_address') }} :{{$customer->isIpaddress()}}
                                            </a>
                                        </li>
                                    @else
                                        <li>
                                            <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.ip_address_confirm', ['user' => $customer->displayName()]) }}" 
                                                href="{{ action('Admin\MailIpAddressController@assignIp', ['user_id' => $customer->user->id]) }}">
                                                <span class="material-symbols-rounded">public</span> {{ trans('messages.ip_address') }}
                                            </a>
                                        </li>                                    
                                    @endif
                                @endif
