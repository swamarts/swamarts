    ...
    <div class="col-md-12">
        <div class="tabbable pb-2">
            <ul class="nav nav-tabs nav-tabs-top nav-underline mb-4">
                ...
                @if (Auth::user()->customer_admin_id == 0)
                    ...
                    <li class="nav-item {{ $menu == 'users' ? 'active' : '' }}">
                        <a href="{{ action("AccountController@users") }}" class="nav-link position-relative {{ isset($tab) && $tab == 'users' ? 'active' : '' }}">
                            <span class="material-symbols-rounded">person</span> {{ trans('messages.users') }}
                        </a>
                    </li>
                    <li class="nav-item {{ $menu == 'UTM' ? 'active' : '' }}">
                        <a href="{{ action("GoogleUTMController@index") }}" class="nav-link position-relative {{ isset($tab) && $tab == 'UTM' ? 'active' : '' }}">
                            <span class="material-symbols-rounded">person</span> {{ trans('messages.utm') }}
                        </a>
                    </li>
                @endif
