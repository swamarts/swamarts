...
class MailList extends Model
{
    ...
    public function export($progressCallback = null, $segment = null)
    {
        ...
        if (!is_null($progressCallback)) {
            $progressCallback($processed, $total, $failed, $message = 'Export is in progress...'); //エクスポートが進行中です
        }
        ...
        cursorIterate($query, $orderBy = 'subscribers.id', $pageSize, function ($subscribers, $page) use ($writer, $listFields, &$processed, &$total, &$failed, &$message, $pageSize, $progressCallback) {
            ...
            // Callback
            if (!is_null($progressCallback)) {
                $progressCallback($processed, $total, $failed = 0, $message = sprintf('Export is in progress エクスポートが進行中です, %s / %s records written', $processed, $total));
            }
        });

        if (!is_null($progressCallback)) {
            $progressCallback($processed, $total, $failed = 0, $message = sprintf('Export complete, processed: エクスポート完了, 処理済み：%s / %s', $processed, $total));
        }
    }
