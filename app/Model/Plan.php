...
class Plan extends Model
{
    ...
    {
        return $this->hasMany(MailIpAddress::class, 'plan_id', 'id');
    }
    ...
    public function ipAddressValue()
    {
        $ip = '';
        foreach ($this->load('ipAddress')->ipAddress as $key => $value) {
            if ($value->user_id == auth()->id()) {
               $ip.='<span>'.$value->ip_address.'</span><br>'; 
            }
        }
        return $ip;
    }
    ...
    */
     ...
     * @return array
    */
    public function displayTotalUsers()
    {
        if ($this->getOption('users_max') == -1) {
            return trans('messages.unlimited');
        } else {
            return number_with_delimiter($this->getOption('users_max'));
        }
    }
     ...
     * @return array
    */
    public function displayTotalSubscribers()
    {
    ...
    public function fillOptions($options = [])
    {
        ...
        if (empty($this->description)) {
            $this->description = '[ plan description goes here ]'; // プランの説明はこちら
        }

        $this->options = json_encode($saveOptions);
    }

    public function fillTier($tier = [])
    {
        $originalData = [];
        for ($i=0; $i < count($tier['price']); $i++) { 
            $originalData[$i]["id"] = $i;
            $originalData[$i]["price"] = $tier["price"][$i];
            $originalData[$i]["subscriber_max"] = $tier["subscriber_max"][$i];
            $originalData[$i]["email_max"] = $tier["email_max"][$i];
            $originalData[$i]["users_max"] = $tier["users_max"][$i];
        }
        $this->tier = json_encode($originalData);
    }
    /**
     * Fill from request.
     */
    public function fillAll($request)
    {
        ...
        if (isset($request->plan['general'])) {
            ...
            if (empty($params['has_ip_address'])) {
                $params['has_ip_address'] = 0;
            }
            $this->fill($params);
        }
        ...
        // if has tier params 
        if (isset($request->plan['tier'])) {
            $this->fillTier($request->plan['tier']);
        }
        ...
        if ($request->old()) {
            ...
            // if (isset($request->old()['plan']['tier'])) {
            //     // fill all tier
            //     $this->fillTier($request->old()['plan']['tier']);
            // }
