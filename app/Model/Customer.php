...
class Customer extends Model
{
    ...
    public function maxSubscribers()
    {
        ...
        if ($count == -1) {
            ...
            if(!empty($this->subscription->user_plan)){
                return !empty(json_decode($this->subscription->user_plan)->subscriber_max) ? json_decode($this->subscription->user_plan)->subscriber_max : $count;
                // return json_decode($this->subscription->user_plan)->subscriber_max;
            }
            return $count;
        }
    }
     ...
     */
    public function assignPlanoldCode($plan,$request = [])
    {
        ...
        if (!empty($request) && $request['tier']) {
            $tier = json_decode($plan->tier)[$request['tier']];
            $tier->currency = $request['currency'];
        }else{
            if (!empty($plan->tier)) {
                $tier = json_decode($plan->tier)[0];
                $tier->currency = $request['currency'];
            }else {
                $tier['currency'] = $request['currency'];
            }
        ...
        // create init invoice
        $subscription->createInitInvoice($request);

        return $subscription;
    }

    public function assignPlan($plan, $request = [])
    {
        if ($this->getNewOrActiveSubscription()) {
            $this->getNewOrActiveSubscription()->update([
                'status' => 'cancelled'
            ]);
        ...
        DB::transaction(function () use ($customer, $plan, &$subscription, $tier) {
            $subscription = Subscription::createNewSubscription($customer, $plan, $tier);
            if ($plan->hasTrial()) {
    ...
    public function isIpaddress(){
        $data = MailIpAddress::where('user_id', $this->user->id)->where('status', 'closed')->first();
        // foreach ($this->getCurrentActiveSubscription()->plan->plansSendingServers as $key => $serverInfo) {
            if(!empty($data)){
