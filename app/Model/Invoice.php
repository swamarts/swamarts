...
class Invoice extends Model
{
    ...
    public const TYPE_NEW_IpAddress = 'ip_address_new';
    ...
    public function process()
    {
        ...
        if (!empty($data['subscription_uid'])) {
        ...
        switch ($this->type) {
            ...
            case self::TYPE_CHANGE_PLAN:
                ...
                $this->assginIpAddress();
                break;
            case self::TYPE_NEW_IpAddress:
                MailIpAddress::where('ip_address', $data['ipaddress'])->update(['user_id' =>  auth()->id()]);
                break;
            default:
    ...
    public function assginIpAddress(){
        $subscription = Request()->user()->customer->getNewOrActiveSubscription();
        if ($subscription->plan->has_ip_address) {
